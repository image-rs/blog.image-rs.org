...
depend one at least one of the crates under our organization. That's mind
boggling.
...
The `jpeg-decoder` crate now correctly decoders all progressive images. It now
...
can get very close with the right code structure. Additionally, a number of
index checks have been identified. Together, the whole decoding process takes
20%-30% less time.
...
fraction of the overall time _and_ blocks all worker threads.
